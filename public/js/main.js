...
(() => {
  ...
  /* Re-queries items at click time (not setup time) so filters work on
     content that is fetched from the API and rendered after page load. */
  document.querySelectorAll('[data-filter-group]').forEach((group) => {
    ...
    buttons.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll(`${targetSelector} [data-filter-tags]`).forEach((item) => {
  ...
  if (lightbox) {
    ...
    /* Event delegation so lightbox works on gallery items rendered
       dynamically from the API, not just ones present at page load. */
  ...
  /* Real API-backed forms: admissions, contact, student/staff auth.
     Usage: data-api-form="/api/..." [data-api-method="PATCH"] [data-redirect="page.html"] */
  document.querySelectorAll('[data-api-form]').forEach((form) => {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch(form.dataset.apiForm, {
        ...
        if (!res.ok) {
          showFormMessage(form, data.error || 'Something went wrong. Please try again.', true);
          return;
        }
        ...
        showFormMessage(form, 'Network error. Please check your connection and try again.', true);
      } finally {
        if (submitBtn) {
  ...
  /* ---------- Site settings hydration (contact email/phone, staff-editable) ---------- */
  (async () => {
    try {
      ...
      if (settings.contact_email) {
        document.querySelectorAll('a[href^="mailto:"]').forEach((el) => {
          ...
          replaceLinkText(el, settings.contact_email);
        });
      }
      if (settings.contact_phone) {
        document.querySelectorAll('a[href^="tel:"]').forEach((el) => {
          el.href = `tel:${settings.contact_phone.replace(/[^\d+]/g, '')}`;
