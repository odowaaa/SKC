...
│   └── img/                         # Crest, full logo lockup, OG image (SVG)
...
```bash
npm install
npm run build   # compiles src/input.css -> assets/css/tailwind.css (minified)
...
Since `assets/css/tailwind.css` is already committed, Cloudflare Pages can also be configured with **no build command** and output directory `/` for a pure static deploy — only re-run `npm run build` locally if you change Tailwind classes or `src/input.css`.
